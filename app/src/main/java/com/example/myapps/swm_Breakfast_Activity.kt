...
import kotlinx.android.synthetic.main.activity_swm__breakfast_.*
import kotlinx.android.synthetic.main.recycle_row_category.view.*

class swm_Breakfast_Activity : AppCompatActivity() {
    val Bcategory: String = "Breakfast"
    val adapter = GroupAdapter<GroupieViewHolder>()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_swm__breakfast_)

        wm_breakfast_back.setOnClickListener {
            onBackPressed()
        }

        wm_breakfast_back_text.setOnClickListener {
            onBackPressed()
        }

        StaggeredGridLayoutManager(2, StaggeredGridLayoutManager.VERTICAL).apply {
            wm_recycle_breakfast.layoutManager = this
        }

        for(i in 0 until 6 step 1){
            adapter.add(loadItem())}
        wm_recycle_breakfast.adapter = adapter
    }
    companion object{
